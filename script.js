...
function openRegistrationFlow(){openTelegramJoin("Starter",10);}
function openRegistrationFlowWithTier(tier){
  const p=IOIS_CONFIG.plans.find(x=>x.id===tier) || IOIS_CONFIG.plans[0];
  openTelegramJoin(p.display || p.name, p.price);
}
function openTelegramJoin(serviceName="IOIS Service", amount="") {
  const planText = amount ? `₹${Number(amount).toLocaleString("en-IN")}` : "";
  const text = `How to Join ${serviceName}${planText ? ` (${planText})` : ""}?`;
  const url = `https://t.me/ioisplatform?text=${encodeURIComponent(text)}`;
  window.open(url, "_blank", "noopener,noreferrer");
}
function openLoginModal(){ /* Public login intentionally removed from the platform UI. */ }
